function [FFP,flag_err]=calc_footprint_FFP_climatology_parallel(zm,z0,umean,h,ol,sigmav,ustar,wind_dir,varargin)
...
% version: 1.41
% last change: 23/02/2021 natascha kljun
%
...
parfor foot_loop = 1:ts_len %parallel computing
...
%--------------------------------------------------------------------
        ...
        xstar_ci_dummy = zeros(size(x_2d));
        px             = ones(size(x_2d));
        if ~isnan(z0l)
            if oll <= 0 || oll >= ol_n
